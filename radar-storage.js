...
export function validateRecord(item,date,method='비로그인 웹'){
...
export async function storageRequest(path,input){
  ...
  const rows=merge?list.filter(r=>r&&!r.demo&&!String(r.id||'').startsWith('seed-')&&AI.has(r.ai)).map(r=>validateRecord({...r,our_mention:r.ourMention,source_url:r.sourceUrl},r.date,r.collectionMethod)) : list.map(r=>validateRecord(r,input.date,input.collection_method));
